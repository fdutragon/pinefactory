...
const codeSnippets = [
	{
		...
		description:
			'Advanced entry logic combining Bollinger Bands extreme levels with RSI oversold/overbought conditions for high-probability reversal trades.',
	...
	},
	{
		...
		description:
			'Protect your profits with an intelligent trailing stop that activates after reaching a profit threshold and follows price movement.',
...
strategy.exit("BUY EXIT", "BUY", 
	...
	},
	{
		...
		description:
			'Dynamically adjust your position size based on account equity and predefined risk levels, with automatic win rate tracking and pause functionality.',
...
qty_calc = round_qty(
	...
	},
	{
		...
		description:
			'Activate simulation mode after a loss to test strategies without risking real capital, with automatic reversion to normal mode upon successful signal.',
...
if simulation_mode and signal != 0
	...
	},
	{
		...
		description:
			'Automatic daily reset of trade metrics with configurable timezone, plus win rate monitoring and strategy pause if performance dips below a set threshold.',
...
if trades_today >= min_trades_daily and 
   win_rate_today < min_win_rate_daily
	...
	},
	{
		...
		description:
			'Seamlessly integrate with Binance using built-in alert messages and payload formatting, ensuring accurate and efficient order execution.',
						...
						{codeSnippets.map((feature, index) => {
							...
							return (
								<motion.button
									...
									whileHover={selectedFeature !== feature.id ? { x: 2 } : {}}
									whileTap={{ scale: 0.98 }}
